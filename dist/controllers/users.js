"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var client_1 = require("@prisma/client");
var usersRouter = require("express").Router();
var UserAuth = require("../helpers/users");
var prisma = new client_1.PrismaClient();
usersRouter.get("/all", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var users;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, prisma.users.findMany()];
            case 1:
                users = _a.sent();
                res.status(200).json(users);
                return [2 /*return*/];
        }
    });
}); });
usersRouter.get("/:id", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, user;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                id = parseInt(req.params.id);
                return [4 /*yield*/, prisma.users.findUnique({
                        where: {
                            id_user: id
                        }
                    })];
            case 1:
                user = _a.sent();
                res.status(200).json(user);
                return [2 /*return*/];
        }
    });
}); });
usersRouter.get("/vehicules/:id", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, vehiculeUser;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                id = parseInt(req.params.id);
                return [4 /*yield*/, prisma.vehicules.findMany({
                        where: {
                            user_id_user: id
                        }
                    })];
            case 1:
                vehiculeUser = _a.sent();
                res.status(200).json(vehiculeUser);
                return [2 /*return*/];
        }
    });
}); });
usersRouter.post("/", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, firstname, lastname, email, password, address, phone, postal_code, city, emailExisting;
    return __generator(this, function (_b) {
        switch (_b.label) {
            case 0:
                _a = req.body, firstname = _a.firstname, lastname = _a.lastname, email = _a.email, password = _a.password, address = _a.address, phone = _a.phone, postal_code = _a.postal_code, city = _a.city;
                return [4 /*yield*/, prisma.users.findUnique({
                        where: {
                            email: email
                        }
                    })];
            case 1:
                emailExisting = _b.sent();
                console.log(emailExisting);
                if (emailExisting === null) {
                    UserAuth.hashPassword(password)
                        .then(function (hashedPassword) {
                        prisma.users
                            .create({
                            data: {
                                firstname: firstname,
                                lastname: lastname,
                                email: email,
                                hashedPassword: hashedPassword,
                                address: address,
                                phone: phone,
                                postal_code: postal_code,
                                city: city
                            }
                        })
                            .then(function (res) { return console.log(res); })["catch"](function (err) { return res.send(err); });
                    })
                        .then(function () {
                        return res.json({
                            firstname: firstname,
                            lastname: lastname,
                            email: email,
                            address: address,
                            phone: phone,
                            postal_code: postal_code,
                            city: city
                        });
                    })["catch"](function (err) { return res.send(err); });
                }
                else {
                    res.status(409).send("Email already used");
                }
                return [2 /*return*/];
        }
    });
}); });
usersRouter.put("/:id", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, userUpdate;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                id = parseInt(req.params.id);
                return [4 /*yield*/, prisma.users.update({
                        where: {
                            id_user: id
                        },
                        data: {
                            firstname: req.body.firstname,
                            lastname: req.body.lastname,
                            email: req.body.email,
                            hashedPassword: req.body.hashedPassword,
                            address: req.body.address,
                            phone: req.body.phone,
                            postal_code: req.body.postal_code,
                            city: req.body.city
                        }
                    })];
            case 1:
                userUpdate = _a.sent();
                res.json(userUpdate);
                return [2 /*return*/];
        }
    });
}); });
usersRouter["delete"]("/:id", function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id, userDelete;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                id = parseInt(req.params.id);
                return [4 /*yield*/, prisma.users["delete"]({
                        where: {
                            id_user: id
                        }
                    })];
            case 1:
                userDelete = _a.sent();
                res.status(200).send("User deleted!");
                return [2 /*return*/];
        }
    });
}); });
module.exports = usersRouter;
//# sourceMappingURL=users.js.map